...
read -r -d '' HELP <<EOF
emailobfuscator test runner. Execute unit test suite and some other details.
...
shift $((OPTIND - 1))
if [ -n "${1}" ]; then
    TEST_FILE="Web/typo3conf/ext/emailobfuscator/${1}"
else
    case ${TEST_SUITE} in
        unit)
            TEST_FILE="Web/typo3conf/ext/emailobfuscator/Tests/Unit"
            ;;
    esac
fi
