...
*  Copyright notice
*
*  (c) 2012 Thomas Pronold (tp@tpronold.de)
*  All rights reserved
*
...
class tx_emailobfuscator {
	...
	public function initEmailObfuscator(Array &$parameters, tslib_cObj &$pObj) {
		...
		if($this->isMailtoTypolink() && $this->isEmailObfuscatorEnabled()) {
	...
	private function isEmailObfuscatorEnabled() {
		if($this->getConfVar('enableEmailObfuscator') == 1) {
			return true;
		}
		return false;
	}
	...
	private function isAddNoscriptEnabled() {
		if($this->getConfVar('addNoscript') == 1) {
			return true;
		}
		return false;
	}
	...
	private function execNonJSObfuscation() {
		if($this->isAddNoscriptEnabled()) {
			$noJavascriptPart = "<noscript>";
	...
	*/
	...
	private static function convertToJSWriteDocument($string) {
		...
		//$javascriptPart = "<script language='JavaScript' type='text/javascript'>" . . "</script>";
		return $javascriptVarDeclaration .  $javascriptDocumentWrite ;
	}


	/**
	 * obfuscates an emailadress with some random methods
	...
	public static function wrapWithSpan($string) {
		return "<span style=\"display:inline;\">".$string."</span>";
	}
	...
	private function getAllowedSelectors() {
		...
		if($this->getConfVar('allowedCSSSelectors')) {
			...
			foreach($allowedSelectorsTemp as $key => $value) {
				$value = trim($value);
				if($value != '') {
